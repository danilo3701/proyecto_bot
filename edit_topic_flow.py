...
async def handle_waiting_section(message: Message, state: FSMContext):
    ...
    if text == "📚 Словарь":
        ...
        if not phases:
            ...
            # после создания фаз пользователь вернётся сюда
...
async def create_edit_vocab_phase(message: Message, state: FSMContext):
    ...
    await state.update_data(current_phase_id=new_id)
...
async def handle_choose_block_type(message: Message, state: FSMContext):
    ...
    if text in block_types:
       ...
       # 💬 если это словарь — считаем по длине выбранной фазы

       # 💬 если это словарь — считаем по длине выбранной фазы
        if main_action == "vocab":
...
async def handle_insert_index(message: Message, state: FSMContext):
    ...
    if action == "quiz":
        await message.answer(
            "📝 Введите текст перед квизом или '-' для пропуска:",
            reply_markup=ReplyKeyboardRemove()
        )
        return await state.set_state(EditTopicStates.waiting_quiz_text)
...
async def handle_quiz_block(message: Message, state: FSMContext):
    # — Сначала прочитаем FSM-данные и очистим их для следующего раза
    data      = await state.get_data()
    quiz_text = data.get("current_quiz_text")
    idx       = data.get("insert_index")
    # очистим переменные
    ...
    if len(parts) != 6:
        return await message.answer(
            "❗ Формат неверный. Нужны 6 частей через `|`. Попробуйте снова."
        )
    ...
    # 💬 добавляем текст перед QUIZ, если он указан
    if quiz_text:
        new_block["text"] = quiz_text

    # 3) Вставляем внутрь выбранной фазы
    ...
    # 5) Ответ и возврат в меню раздела
    kb = section_menu()
...
async def handle_vocab_textquiz_answer(message: Message, state: FSMContext):
    ...
    # 💬 вставляем TextQuiz в выбранную фазу
    cp         = data["current_phase_id"]
    phase_list = topic["vocab"][cp-1]["vocab"]
    phase_list.insert(min(max(idx,0), len(phase_list)), new_block)
    topic["vocab"][cp-1]["vocab"] = phase_list
