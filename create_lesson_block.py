...
LEVELS = ["A0","A1", "A2", "B1", "B2", "C1"]  # 💬 список уровней
...
class NewTopicStates(StatesGroup):
    ...
    waiting_vocab_photo_quiz_block = State()  #
    ...
    if last == "vocab":
        rows = [
            ...
            [KeyboardButton(text="📘VOC"),
             KeyboardButton(text="🔤🎲TXT_QUIZ"),
             KeyboardButton(text="➕QUIZ")],
            [KeyboardButton(text="📥TXT_QUIZ"),
             KeyboardButton(text="📥QUIZ"),
             KeyboardButton(text="EXTRA_QUIZ")],
...
#Создание extra_quiz
@router.message(NewTopicStates.waiting_post_action, F.text=="EXTRA_QUIZ")
...
@router.message(NewTopicStates.waiting_post_action, F.text == "🔤🎲TXT_QUIZ")
...
async def save_vocab_textquiz_block(message: Message, state: FSMContext):
    ...
    # 💬 сохраняем TextQuiz в выбранной фазе
    data       = await state.get_data()
...
async def handle_post_action(message: Message, state: FSMContext):
    ...
    if last_block == "vocab":
        if text == "📘VOC":
            ...
            if not phases:
                ...
                # убираем старое меню и показываем кнопки выбора фазы
                await message.answer("Выберите фазу или создайте новую:", reply_markup=ReplyKeyboardRemove())
                buttons = [
                    [KeyboardButton(text=f"{p['phase_id']}. {p['phase_name']}")]
                    for p in phases
                ]
    ...
    # ─── Некорректный ввод ───
    await message.answer(
        "❗ Пожалуйста, нажми одну из кнопок: «📘VOC», «➕ Добавить QUIZ» или «↩️ Вернуться в Главное меню»."
    )
...
async def save_vocab_quiz_block(message: Message, state: FSMContext):
    # 1) Разбор и валидация формата
    parts = [p.strip() for p in message.text.split("|")]
    if len(parts) != 6:
        return await message.answer("❗ Формат неверный — нужно 6 частей через `|`. Попробуй снова.")
    question, correct, w1, w2, exp_corr, exp_wrong = parts

    # 2) Подготовка опций и нового блока
    import random
    options = [correct, w1, w2]
    random.shuffle(options)

    data = await state.get_data()
    topic = data["topic"]                                    # 💬 получаем тему из state
    new_quiz = {
        "type": "quiz",
        "question": question,
        "options": options,
    ...
    # 3) Добавляем опциональный текст
    quiz_text = data.get("current_quiz_text")
    if quiz_text:
        new_quiz["text"] = quiz_text
    await state.update_data(current_quiz_text=None)           # 💬 очищаем временное поле

    # 💬 сохраняем Quiz в выбранной фазе
    data       = await state.get_data()
    cp         = data["current_phase_id"]
    topic      = data["topic"]
    topic["vocab"][cp-1]["vocab"].append(new_quiz)
    ...
    # 6) Отправляем подтверждение и меню
    await message.answer("✅ Викторина сохранена.", reply_markup=ReplyKeyboardRemove())
    await send_post_menu(message, state)
...
async def save_vocab_text_block(message: Message, state: FSMContext):
    ...
    with open(data["topic_path"], "w", encoding="utf-8") as f:
        json.dump(topic, f, ensure_ascii=False, indent=2)
    # 3) Переходим к опциональному quiz
    await message.answer(
        "📝 Введите quiz для этого текста через `|`: вопрос|правильный|неправ1|неправ2|объяснениеПравильного|объяснениеНеправильного\n"
        "или просто '-' для пропуска.",
        reply_markup=ReplyKeyboardRemove()
    )
    return await state.set_state(NewTopicStates.waiting_vocab_text_quiz)


# ——— Опциональный Quiz после TEXT ———
@router.message(NewTopicStates.waiting_vocab_text_quiz)
@router.message(NewTopicStates.waiting_vocab_text_quiz)
async def ask_vocab_text_quiz_block(message: Message, state: FSMContext):
    text = message.text.strip()
    # — Пропуск квиза
    if text == "-":
        await message.answer("Квиз пропущен.", reply_markup=ReplyKeyboardRemove())
    ...
    cp    = data["current_phase_id"]
    topic = data["topic"]
    block = topic["vocab"][cp-1]["vocab"][-1]
    ...
    await message.answer("✅ Квиз к тексту сохранён.", reply_markup=ReplyKeyboardRemove())
    await send_post_menu(message, state)


'''
@router.message(NewTopicStates.waiting_vocab_text_quiz_block)
async def save_vocab_text_quiz_block(message: Message, state: FSMContext):
    parts = (await state.get_data()).get("current_text_quiz_parts", [])
    question, correct, w1, w2, exp_corr, exp_wrong = parts
    options = [correct, w1, w2]
    import random; random.shuffle(options)
    ...
    # берём только что добавленный текстовый блок
    block = topic["vocab"][cp-1]["vocab"][-1]
    # сохраняем quiz внутрь блока
    block["quiz"] = {
        "question":           question,
        "options":            options,
        "correct_answer":     correct,
    ...
    # чистим временные данные и сохраняем в файл
    await state.update_data(current_text_quiz_parts=None)
    with open(data["topic_path"], "w", encoding="utf-8") as f:
        json.dump(topic, f, ensure_ascii=False, indent=2)

    await message.answer("✅ Quiz к тексту сохранён.", reply_markup=ReplyKeyboardRemove())
    await send_post_menu(message, state)
'''
...
async def receive_vocab_media(message: Message, state: FSMContext):
    ...
    # 1) подтверждаем сохранение медиа
    await message.answer("Фото/медиа сохранено.", reply_markup=ReplyKeyboardRemove())
    # 2) просим опциональный quiz или '-' для пропуска
    await message.answer(
        "📝 Введите quiz для этого фото через `|`,\n"
        "в формате:\n"
        "Вопрос|Правильный|Неправ1|Неправ2|ПояснениеПравильного|ПояснениеНеправильного\n"
        "или просто `-` для пропуска."
    )
    await state.set_state(NewTopicStates.waiting_vocab_photo_quiz)



@router.message(NewTopicStates.waiting_vocab_photo_quiz)
async def ask_vocab_photo_quiz_block(message: Message, state: FSMContext):
    text = message.text.strip()
    # — Пропуск квиза
    if text == "-":
        await message.answer("Квиз пропущен.", reply_markup=ReplyKeyboardRemove())
    ...
    await message.answer("✅ Квиз к фото сохранён.", reply_markup=ReplyKeyboardRemove())
    await send_post_menu(message, state)


"""

@router.message(NewTopicStates.waiting_vocab_photo_quiz_block)
async def save_vocab_photo_quiz_block(message: Message, state: FSMContext):
    parts = (await state.get_data()).get("current_photo_quiz_parts", [])
    question, correct, w1, w2, exp_corr, exp_wrong = parts
    options = [correct, w1, w2]
    import random; random.shuffle(options)
    ...
    cp         = data["current_phase_id"]
    topic      = data["topic"]
    # берём только что добавлённый последний media‐блок
    block = topic["vocab"][cp-1]["vocab"][-1]
    # сохраняем quiz прямо внутрь блока
    block["quiz"] = {
        "question": question,
        "options": options,
        "correct_answer": correct,
    ...
    # сбрасываем временные данные и перезаписываем JSON
    await state.update_data(current_photo_quiz_parts=None)
    with open(data["topic_path"], "w", encoding="utf-8") as f:
        json.dump(topic, f, ensure_ascii=False, indent=2)

    await message.answer("✅ Quiz к фото сохранён.", reply_markup=ReplyKeyboardRemove())
    await send_post_menu(message, state)
