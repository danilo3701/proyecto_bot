...
### 2026-02-16 · grammar/menu · core8_1 · P
Контекст: inline-кнопка «Назад» (callback `menu:back`) зависает/не обрабатывается
Симптом: “вечная загрузка” на callback, если state уже очищен или не совпадает с StateFilter
Сделали:
- Добавили глобальный callback-хендлер `menu:back` без StateFilter
- В хендлере всегда делаем `callback.answer()`, чистим FSM и возвращаем в `start_handler`
Файлы:
- core8_1.py
Коммит: PENDING
Результат: callback `menu:back` должен перестать зависать и всегда возвращать пользователя в стартовое меню
Следующий шаг: нажать «Назад» из грамматики/любой ветки с `menu:back` и убедиться, что нет “часиков” и открывается стартовое меню
