...
def get_vocab_list(data: dict) -> list:
    """
    💬 Компилируем список блоков фазы с подмешиванием quiz_pool и textquiz_pool.
       Правила:
       • После КАЖДОГО link: чередуем Quiz → TextQuiz → Quiz → TextQuiz...
       • Кол-во на один link = floor(total_pool / links) для каждого пула
       • Остатки пулов кладём В КОНЕЦ фазы (чередуя Quiz/TextQuiz)
       • Inline-квизы у text/photo (поле 'quiz') не трогаем
       • Если ссылок нет — показываем исходные блоки + хвост из пулов (Q/T, чередуя)
    """
    topic_key = data.get("selected_topic")
    topic     = topics.get(topic_key, {})
    ph_id     = data.get("selected_phase_id")

    # если фаза не выбрана — как раньше (legacy)
    if not ph_id:
    ...
    base      = list(phase.get("vocab", []))
    quiz_pool = list(phase.get("quiz_pool", []))
    text_pool = list(phase.get("textquiz_pool", []))

    link_pos = [i for i, b in enumerate(base) if ("link" in b) or ("url" in b)]
    L = len(link_pos)

    # helper: альтернирующая склейка списков A,B начиная с A
    def alt_merge(A, B):
        out = []
        ai = bi = 0
    ...
    # 1) нет ссылок — исходный порядок + все из пулов в хвост
    if L == 0:
        tail = alt_merge(quiz_pool, text_pool)   # 💬 Q/T чередуем
        return base + tail

    # 2) теперь после КАЖДОГО link вставляем РОВНО 2 блока из пулов
    #    (quiz и/или textquiz). Если какого-то типа не хватает — добираем другим.
    from collections import deque
    qdq, tdq = deque(quiz_pool), deque(text_pool)   # 💬 очереди для удобного .popleft()

    # 3) собираем: после каждого link — кладём ДВА элемента
    compiled = []
    link_idx = 0
    for idx, block in enumerate(base):
        compiled.append(block)
        if link_idx < L and idx == link_pos[link_idx]:
            pair = []

            # 💬 Чередуем стартовый приоритет: на чётных ссылках сначала quiz, на нечётных — textquiz
            first_is_quiz = (link_idx % 2 == 0)
            order = ("quiz", "textquiz") if first_is_quiz else ("textquiz", "quiz")

            for pref in order:
                if pref == "quiz":
                    if qdq:
                        pair.append(qdq.popleft())
                    elif tdq:
                        pair.append(tdq.popleft())
                else:  # textquiz
                    if tdq:
                        pair.append(tdq.popleft())
                    elif qdq:
            ...
            # 💬 Если удалось взять только один — попробуем добрать ещё один из любого пула
            while len(pair) < 2:
                if qdq:
                    pair.append(qdq.popleft())
                elif tdq:
            ...
            compiled.extend(pair)
            link_idx += 1

    # 4) остатки пулов — в конец, также чередуя
    compiled.extend(alt_merge(list(qdq), list(tdq)))
...
async def verify_subscription_for_topic(user_id: int, channel: str) -> bool:
    # 💬 проверяем подписку только на один заданный канал
    required = [channel] if channel else []
    ok = True
    for ch in required:
        try:
        ...
        if member.status not in ("member", "administrator", "creator"):
            ok = False
            # 💬 фиксируем отписку в user_data.json
            data     = load_user_data()
            u        = data.setdefault(str(user_id), {})
            sessions = u.setdefault("channels", {}).get(ch, [])
...
async def level_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    if not buttons:
        await callback.message.edit_text(f"🤷‍♂️ Тем пока нет на уровне. Скоро добавим!")
        # 🚀 Возвращаемся к выбору категории сразу после уведомления
        return await start_handler(callback.message, state)

    inline_keyboard = [buttons[i:i+2] for i in range(0, len(buttons), 3)]
...
async def topic_chosen(query: CallbackQuery, state: FSMContext):
    ...
    # 💬 Показываем pop-up с описанием темы (не более ~200 символов)
    desc = topics.get(query.data.split(":",1)[1], {}).get("description", "")
    if desc:
        # Telegram alert ограничен длиной текста
        MAX_ALERT = 200
    ...
    # 💬 Получаем следующий канал для подписки из внешнего файла
    ...
    next_idx = (last_idx + 1) % len(channels_list) if channels_list else None
    channel = channels_list[next_idx] if channels_list else None
    u["last_subscription_channel_index"] = next_idx
    save_user_data(all_user_data)

    # 💬 Сохраняем канал в state и формируем required
    ...
    if not sessions:
        ...
        await query.message.answer(
            f"🔒 Чтобы получить доступ, подпишись на: {channels_str}",
            reply_markup=check_subscription_kb(topic_key)
        )
        # 💬 Стикер «покажись» запускается в фоне и не блокирует кнопку
        asyncio.create_task(send_and_auto_delete_sticker(bot, query.message.chat.id, "CAACAgIAAxkBAAE5B0xokoDr9gxJWIZKzvdUCAfYTFuSQwACuQAD9wLID5Ahqvg5d0bYNgQ", delay=10))

        await state.set_state(LessonStates.waiting_subscription)
        return

    # 2.2) Повторная проверка отписок на канале
    channel = (await state.get_data()).get("required_channel")
    if not await verify_subscription_for_topic(query.from_user.id, channel):
...
async def lesson_menu_handler(message: Message, state: FSMContext):
    ...
    # 5) Общее XP
    # 💬 показываем ГЛОБАЛЬНЫЙ XP из xp_data (а не сессионный xp в state)
    total_xp_user = load_xp_data().get(str(message.chat.id), {}).get("total_xp", 0)
    parts.append(f"🏆 <b>Всего опыта: {total_xp_user} XP</b> 🌟")
...
async def send_one_extra_vocab_quiz(message: Message, state: FSMContext):
    ...
    # 💬 без explanation_correct — по общей политике (объяснение только при ошибке в основных квизах)
...
async def send_one_vocab(message: Message, state: FSMContext):
    ...
    # 💬 Отправляем название и кнопку-ссылку
    await bot.send_message(
        message.chat.id,
        f"<b>{title}</b>",
        parse_mode="HTML",
...
async def ad_reaction_handler(callback: CallbackQuery, state: FSMContext):
    ...
    # 1) Удаляем вопрос с кнопками
    chat_id = callback.message.chat.id
    q_id = data.get("current_ad_question_id")
    if q_id:
        try:
            await bot.delete_message(chat_id, q_id)
        except:
            pass

    # 2) Показываем реакцию
    reaction_msg = await callback.message.answer(reaction)
    await asyncio.sleep(1.5)


    chat_id = callback.message.chat.id
    # 2) Удаляем рекламу, вопрос и реакцию
    for mid in (
        data.get("current_ad_msg_id"),
        data.get("current_ad_question_id"),
        reaction_msg.message_id
    ):
        if mid:
            try:
...
# — Отправляем Quiz через bot, чтобы не вызывать методы у ChatFullInfo —
    ...
    poll_message = await bot.send_poll(
        ...
        open_period=12,
        is_anonymous=False,
        explanation=block.get("explanation_wrong", "")  # 💬 текст для неправильного ответа
    )
...
async def handle_vocab_textquiz_answer(message: Message, state: FSMContext):
    ...
    # 🔐 Проверяем разблокировку по ГЛОБАЛЬНОМУ XP по теме (до return)
    data2 = await state.get_data()
    threshold = data2.get("xp_threshold", 0)
    topic_xp = load_xp_data().get(str(user_id), {}).get("by_topic", {}).get(topic_key, 0)
    if not data2.get("unlocked") and topic_xp >= threshold:
        await state.update_data(unlocked=True)
