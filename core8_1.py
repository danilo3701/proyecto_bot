...
async def mywords_menu(message: Message, state: FSMContext):
    ...
    user_id = str(message.chat.id)
    await MYWORDS_REPOSITORY.mutate(lambda data: ensure_my_words_user(data, user_id), save=True)  # 💬 гарантируем файл в Volume
...
async def mywords_settings_wait_number(message: Message, state: FSMContext):
    ...
    await MYWORDS_REPOSITORY.mutate(
        lambda data: ensure_my_words_user(data, user_id).setdefault("settings", {}).update({"session_words": n}),
        save=True,
    )
...
async def mywords_delete_confirm_cb(callback: CallbackQuery, state: FSMContext):
    ...
    deleted = await MYWORDS_REPOSITORY.mutate(
